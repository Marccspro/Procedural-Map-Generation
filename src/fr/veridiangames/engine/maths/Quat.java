...
public class Quat {
	...
	public static Quat euler(float x, float y, float z)
	{
        ...
        angle = z * 0.5f;
        sinPitch = Mathf.sin(Mathf.toRadians(angle));
        cosPitch = Mathf.cos(Mathf.toRadians(angle));
        
        angle = y * 0.5f;
        sinRoll = Mathf.sin(Mathf.toRadians(angle));
        cosRoll = Mathf.cos(Mathf.toRadians(angle));

		angle = x * 0.5f;
        sinYaw = Mathf.sin(Mathf.toRadians(angle));
        cosYaw = Mathf.cos(Mathf.toRadians(angle));
